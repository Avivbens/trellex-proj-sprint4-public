    ...
    actions: {
        ...
        async loadBoard({ commit, dispatch }, { boardId }) {
            try {
                ...
                // Send to homepage and clear saved data if trying to access un-authorized board
                if (error.response.status === 401 || error.response.status === 403) {
                    console.log('in error')
                    // await this.$store.dispatch({ type: 'logout' })
                    router.push('/')
                    commit({ type: 'setMsgTxt', txt: 'Access denied for the following board!' })
                    dispatch({ type: 'activeMsg', time: 8000 })
                    localStorage.clear()
                    return
                }

                // if there is no connection - data will load from local storage
